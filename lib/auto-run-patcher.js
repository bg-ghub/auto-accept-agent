...
function analyzeBrowserAction(content, label) {
    ...
    // Extract the confirm callback variable from the confirm:!0 pattern
    // b=Ce(()=>{a(ur(tE,{...confirm:!0...}))}, [...])
    const confirmMatch = chunk.match(/(\w+)=\w+\(\(\)=>\{\w+\(\w+\(\w+,\{[^}]*confirm:!0/);
    if (!confirmMatch) return null;
    const confirmFn = confirmMatch[1];

    // Extract the URL prop variable: url:r in destructuring
    // ({sourceTrajectoryStepInfo:e,shortcuts:t,url:r})
    const urlMatch = chunk.match(/url:(\w+)[,}]/);
    ...
    // Guard on URL being defined, then call confirm
    const insertAt = returnIdx;
    const patchCode = `${BROWSERACTION_MARKER}if(${urlVar})setTimeout(()=>${confirmFn}(),0);`;
    ...
    return {
        ...
        label,
        details: { confirmFn, urlVar, type: 'browserAction' }
    };
}
