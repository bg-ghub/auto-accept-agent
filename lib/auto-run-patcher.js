...
function analyzeFile(content, label) {
    ...
    const policyRe = new RegExp(`([\\w$]+)=[\\w$]+\\?\\.terminalAutoExecutionPolicy\\?\\?${enumAlias.replace(/\$/g, '\\$')}\\.OFF`);
...
function analyzeAutoScroll(content, label) {
    ...
    for (let i = replacements.length - 1; i >= 0; i--) {
        ...
        // Replace the entire "autoScroll:n=>n!==ia.Ask" with "autoScroll:/*AAA:autoscroll*/!0"
