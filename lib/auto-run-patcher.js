...
const BROWSERJS_MARKER = '/*AAA:browserjs*/';
...
function findAntigravityPath() {
    ...
    if (process.platform === 'win32') {
        candidates.push(
            // Prefer IDE (VS Code fork) over standalone when both exist
        ...
        candidates.push(
            ...
            path.join(os.homedir(), 'Applications', 'Antigravity IDE.app', 'Contents', 'Resources'),
        ...
        candidates.push(
            ...
            '/opt/antigravity-ide',
            '/opt/antigravity',
            path.join(os.homedir(), '.local', 'share', 'antigravity-ide'),
...
function analyzeFileAccess(content, label, options) {
    ...
    // Extract the permission function name and enum from the "Allow" button:
    // Old (AG ≤2.0.1): onClick:()=>{PERM_FN(!0,ENUM.CONVERSATION)}
    // New (AG ≥2.0.2): onClick:()=>PERM_FN(!0,ENUM.CONVERSATION)  (zon component, no braces)
...
/**
 * Patch 6: Auto-confirm browser JS execution permission.
 *
 * AG 2.0 shows "Agent needs permission to execute JavaScript on X" with
 * Always run / Deny / Allow buttons. When "Always run" (TURBO) is selected,
 * the onChange handler confirms — but if it's ALREADY set to TURBO on mount,
 * the component still shows the dialog without auto-confirming.
 *
 * Same bug pattern as the old terminal autorun issue.
 *
 * We inject code after the component's return statement opening that
 * auto-confirms when policy is TURBO.
 */
function analyzeBrowserJsPermission(content, label) {
    // Find the unique "execute JavaScript" text anchor
    const anchor = 'Agent needs permission to execute JavaScript';
    const anchorIdx = content.indexOf(anchor);
    if (anchorIdx === -1) return null;

    // Get the component context (the component is ~2000 chars before the anchor)
    const searchStart = Math.max(0, anchorIdx - 2500);
    const chunk = content.substring(searchStart, anchorIdx + 500);

    // Find the TURBO enum and confirm function:
    // Pattern: POLICY_SETTER===ENUM.TURBO&&CONFIRM_FN()
    // e.g., w===vh.TURBO&&y()  or  k===FC.TURBO&&g()
    const turboMatch = chunk.match(/([\w$]+)===([\w$]+)\.TURBO&&([\w$]+)\(\)/);
    if (!turboMatch) return null;
    const confirmFn = turboMatch[3];
    const enumAlias = turboMatch[2];

    // Find the policy variable: browserJsExecutionPolicy??ENUM.ALWAYS_ASK
    // e.g., i?.browserJsExecutionPolicy??vh.ALWAYS_ASK
    const policyMatch = chunk.match(/browserJsExecutionPolicy\?\?([\w$]+)\.ALWAYS_ASK/);
    if (!policyMatch) return null;

    // Find the policy variable name from the broader pattern:
    // VAR=HANDLER?.browserJsExecutionPolicy??ENUM.ALWAYS_ASK
    const policyVarMatch = chunk.match(/([\w$]+)=[\w$]+\?\.browserJsExecutionPolicy\?\?[\w$]+\.ALWAYS_ASK/);
    if (!policyVarMatch) return null;
    const policyVar = policyVarMatch[1];

    // Find a suitable guard to insert after. The component checks for waitingReason:
    // We'll insert before the return statement of the component
    // Find: containerRef:VAR}):  (near end of component before return)
    // Better: insert right before the final return statement
    // Look for the Ft(null) or gi(null) ref creation — it's the last variable before return
    const refMatch = chunk.match(/([\w$]+)=(?:Ft|gi)\(null\);return/);
    if (!refMatch) return null;
    const refStr = refMatch[0];
    const refIdx = chunk.indexOf(refStr);
    if (refIdx === -1) return null;

    // Insert point: right after the ref assignment, before 'return'
    const insertGlobal = searchStart + refIdx + refMatch[1].length + '='.length;
    // Actually, insert right before 'return' in the matched string
    const returnOffset = refStr.indexOf('return');
    const insertAt = searchStart + refIdx + returnOffset;

    // Patch: if policy is TURBO, auto-confirm via setTimeout and return null (hide UI)
    const patchCode = `${BROWSERJS_MARKER}if(${policyVar}===${enumAlias}.TURBO){setTimeout(()=>${confirmFn}(),0);return null;}`;

    return {
        insertAt,
        patchCode,
        patchMarker: BROWSERJS_MARKER,
        label,
        details: { confirmFn, enumAlias, policyVar, type: 'browserJs' }
    };
}

// ─── File Operations ────────────────────────────────────────────────────────

const ALL_MARKERS = [PATCH_MARKER, FILEACCESS_MARKER, AUTOEXPAND_MARKER, AUTOSCROLL_MARKER, BROWSERJS_MARKER];
...
async function patchFile(filePath, label, options) {
    try {
        ...
        // Apply browser JS auto-confirm patch
        content = stripBrowserJsPatch(content);
        const bjsAnalysis = analyzeBrowserJsPermission(content, label);
        if (bjsAnalysis) {
            content = content.substring(0, bjsAnalysis.insertAt) + bjsAnalysis.patchCode + content.substring(bjsAnalysis.insertAt);
            totalBytes += bjsAnalysis.patchCode.length;
            applied.push('browserjs');
        }
...
/** Strip browserjs marker and code */
function stripBrowserJsPatch(content) {
    let c = content;
    while (c.includes(BROWSERJS_MARKER)) {
        const idx = c.indexOf(BROWSERJS_MARKER);
        // Find end: return null;} after the marker
        const afterMarker = c.substring(idx, idx + 500);
        const endMatch = afterMarker.match(/return null;\}/);
        if (!endMatch) {
            // Fallback: strip to next semicolon
            const semi = c.indexOf(';', idx + BROWSERJS_MARKER.length);
            if (semi === -1) break;
            c = c.substring(0, idx) + c.substring(semi + 1);
        } else {
...
/** Strip all patches (autorun, fileaccess, autoexpand, autoscroll, browserjs) */
function stripMarkers(content) {
    return stripBrowserJsPatch(stripAutoscrollPatch(stripAutoexpandPatch(stripAutorunPatch(stripFileaccessPatch(content)))));
}
