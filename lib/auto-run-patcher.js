 ...
 * We inject setTimeout(()=>alwaysAllowFn(), 0) before the return to auto-click "Always Allow".
 */
function analyzeBrowserAction(content, label) {
    ...
    // Find the return statement: return u?r?A($Fi,{onAddToWhitelist:VAR,...
    const searchEnd = Math.min(content.length, baIdx + 1500);
    const chunk = content.substring(baIdx, searchEnd);
    
    // Extract the "Always Allow" handler from onAddToWhitelist:VAR
    const alwaysMatch = chunk.match(/onAddToWhitelist:(\w+)/);
    if (!alwaysMatch) return null;
    const alwaysFn = alwaysMatch[1];

    // Find the return statement to inject before
    const returnPattern = /return \w+\?\w+\?/;
    const returnMatch = chunk.match(returnPattern);
    if (!returnMatch) return null;
    const returnIdx = content.indexOf(returnMatch[0], baIdx);
    if (returnIdx === -1) return null;

    const insertAt = returnIdx;
    const patchCode = `${BROWSERACTION_MARKER}setTimeout(()=>${alwaysFn}(),0);`;
    ...
    return {
        ...
        label,
        details: { alwaysFn, type: 'browserAction' }
    };
}
