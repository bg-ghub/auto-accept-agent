...
function analyzeFileAccess(content, label) {
    ...
    // Safe paths: workspace dirs (Documents), temp, AG config
    // Uses t.absolutePathUri which is available in the component scope.
    const patchCode = `${FILEACCESS_MARKER}setTimeout(()=>{const _u=(t.absolutePathUri||'').toLowerCase();const _ok=_u.includes('/documents/')||_u.includes('\\\\documents\\\\')||_u.includes('/tmp/')||_u.includes('\\\\tmp\\\\')||_u.includes('/temp/')||_u.includes('\\\\temp\\\\')||_u.includes('/.gemini/')||_u.includes('\\\\.gemini\\\\');_ok&&${permFn}(!0,${enumName}.CONVERSATION)},0);`;
