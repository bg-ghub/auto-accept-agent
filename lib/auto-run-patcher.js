...
function analyzeAutoScroll(content, label) {
    // Match pattern: autoScroll:VARNAME=>VARNAME!==
    // This is the function that checks mode !== Ask
    const pattern = /autoScroll:(\w)=>\1!==/g;
    let match;
    const replacements = [];
    ...
    for (let i = replacements.length - 1; i >= 0; i--) {
        ...
        // Replace "autoScroll:n=>n!==" with "autoScroll:!0,n=>n!=="
        // We insert /*AAA:autoscroll*/!0, before the original function
        // so it becomes autoScroll:/*AAA:autoscroll*/!0,n=>n!==
        // (the original function becomes a dead expression after the comma operator)
        patched = patched.substring(0, r.start) +
            `autoScroll:${AUTOSCROLL_MARKER}!0` +
...
/** Strip autoscroll marker and replacement */
function stripAutoscrollPatch(content) {
    // The patch replaces "autoScroll:n=>n!==" with "autoScroll:/*AAA:autoscroll*/!0"
    // To revert: replace "/*AAA:autoscroll*/!0" (which removed the original function) 
    // We can't perfectly restore the original variable name, but we can remove our patch.
    // Since we cut off at the !==, we need to restore: "autoScroll:" + some var + "=>" + var + "!=="
    // Simplest: just remove the marker, leaving autoScroll:!0 (which is functionally fine)
    let c = content;
